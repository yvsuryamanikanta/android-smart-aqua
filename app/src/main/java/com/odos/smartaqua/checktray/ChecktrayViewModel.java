...
public class ChecktrayViewModel extends ViewModel implements ServiceAsyncResponse {
    private Context _context;
    private ActivityAddChecktrayBinding _activityAddChecktrayBinding;
    private ServiceAsyncResponse serviceAsyncResponse;
    private ArrayList<UserRoles> userRolesArrayList;
    private int tankId =-1;

    public ChecktrayViewModel(Context context, ActivityAddChecktrayBinding activityAddChecktrayBinding) {
        this._context = context;
        this._activityAddChecktrayBinding = activityAddChecktrayBinding;
    ...
    public void onSaveClick(View v) {
        if(tankId ==-1){
            ...
            postparams.put("checktrayname", _activityAddChecktrayBinding.edtChecktrayName.getText().toString());
            VolleyService.volleyservicePostRequest(_context, _context.getString(R.string.jsonobjectrequest),
                    ServiceConstants.SAVE_CHECKTRAY, postparams, Helper.headerParams(_context), (ServiceAsyncResponse) serviceAsyncResponse, 2, true);
        ...
        switch (serviceno) {
                    ...
                    if (status.equalsIgnoreCase("Sucess")) {
                        if (!response.equalsIgnoreCase("null")) {
                            ...
                            if (jsonArray.length() != 0) {
                                ...
                                _activityAddChecktrayBinding.spinTanklist.setAdapter(userRolesAdapter);
                                _activityAddChecktrayBinding.spinTanklist.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
                                    @Override
            ...
            case 2:
                try {
                    String status = jsonObject.getString("status");
                    if (status.equalsIgnoreCase("Sucess")) {
                        Helper.showMessage(_context, "Chaecktray saved", AquaConstants.FINISH);
                    }
                } catch (Exception e) {
